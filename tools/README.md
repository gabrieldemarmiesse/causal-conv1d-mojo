...
| Tool | What it does |
|---|---|
| `./tools/bench [args]` | Run `bench_gpu_kernel_time.py` with the GPU lock applied. Supports `--kind {fwd,bwd,update,all}`, `--impl {mojo,upstream,both}`, `--shape B,D,L,W` (or B,D for update), `--iters`, `--warmup`. |
| `./tools/dump_isa --sub <fwd\|bwd_full\|update> --shape <...>` | Patch the relevant `launch.mojo` to set `dump_asm`/`dump_llvm`, JIT-compile **one** variant, dump AMDGPU ISA to `/tmp/mojo_<sub>_<modname>.s` and LLVM IR to `.ll`. Auto-unpatches on exit. |
...
| `./tools/rocprof_pmc --kernel <regex> [bench args]` | Collect MI300X HW counters (VALU/SALU/LDS/VMEM/TCC) for kernels matching `<regex>`. Same upstream-only constraint. |
| `./tools/gpu_run <cmd...>` | Generic flock wrapper. Use for ad-hoc commands that touch the GPU. |
| `./tools/quick_test <fwd\|bwd\|update> '<pytest -k expr>'` | Run a *targeted* subset of the pytest suite — pick a `-k` expression that selects at most ~3 cases. |
| `./tools/hint <agent-name>` (stdin = body) | Append a perf hint to `/tmp/hints.md`. Use for *validated* wins so other agents can see them. |

## The fast iteration loop

```bash
# 1. Baseline: just the kernel you care about, one shape.
./tools/bench --kind fwd --impl both --shape 1,1024,2048,4 --iters 50

# 2. Edit the kernel/launch source.
# 3. Re-measure mojo only (upstream is cached via baselines/).
./tools/bench --kind fwd --impl mojo --shape 1,1024,2048,4 --iters 50

# 4. If the change looks good across a few shapes, do correctness check:
./tools/quick_test fwd 'width4 and fp16 and seqlen_2048'

# 5. If validated, write a hint so the other agents see it.
./tools/hint $YOUR_NAME <<'EOF'
...
# → /tmp/mojo_fwd_<hash>.s   (AMDGPU SASS)
# → /tmp/mojo_fwd_<hash>.ll  (LLVM IR pre-codegen)
...
- **rocprofv3 vs Mojo:** rocprofv3 intercepts HSA at process startup,
  ...
  For Mojo per-kernel time, `./tools/bench` (which uses `torch.profiler`)
