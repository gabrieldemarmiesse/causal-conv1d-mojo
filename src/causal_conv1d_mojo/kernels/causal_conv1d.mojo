...
struct CausalConv1dFn[
    ...
    Computes for each (b, d, t):
        out[b, d, t] = sum_{k=0..W-1} src[b, d, t + k - (W-1)] * weight[d, k]
    where src is `initial_states` concatenated with `x` along the sequence dim
    (or zero-extension when `has_initial_states` is False), with optional bias
    and SiLU activation applied pointwise.

    Also produces `final_states[b, d, s]` = src[b, d, L + s] for s in [0, W-1).
    """
