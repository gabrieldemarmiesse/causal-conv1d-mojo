...
Cache layout (post-PR):
    ...
    $XDG_CACHE_HOME/flash_attn_mojo/<subpkg>/<backend>[/<arch>]/<mod>.hash-<h>.so
...
def cache_dir_for(subpkg: str, backend: str, backend_arch: str = "") -> Path:
    ...
    if backend == "cpu":
        return _CACHE_HOME / "flash_attn_mojo" / subpkg / "cpu" / cpu_tag
    return (
        _CACHE_HOME / "flash_attn_mojo" / subpkg / backend / backend_arch / cpu_tag
    )
    ...
    *,
    ...
    if not so_path.is_file():
        ...
        if os.environ.get("CAUSAL_CONV1D_USE_CACHE_ONLY"):
            ...
            raise RuntimeError(
                f"flash_attn_mojo: cache miss for {subpkg} variant "
        ...
        for old in cache_dir.glob(f"{mod_name}.hash-*.so"):
            old.unlink()
        print(
            f"[flash_attn_mojo] compiling {subpkg} variant {mod_name} — "
...
def _cpu_brand() -> str:
    ...
    raise RuntimeError(
        ...
        "Open an issue at "
        "https://github.com/gabrieldemarmiesse/flash-attn-mojo/issues "
...
def _cpu_microarch_tag() -> str:
    ...
    that ``ls ~/.cache/flash_attn_mojo/<sub>/cpu/`` is informative.
...
def _ptxas_signature() -> str:
    ...
    ``flash_attn_mojo/__init__.py`` so we don't accidentally treat
