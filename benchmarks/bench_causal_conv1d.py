...
def main() -> None:
    ...
    print(
        f"GPU: {torch.cuda.get_device_name(0)} | dtype={args.dtype} | activation={args.activation} | bias={args.bias}\n"
    )
    ...
    for batch, dim, seqlen, width in SHAPES:
        ...
        bias = (
            torch.randn(dim, generator=g).to(device=device, dtype=dtype)
            if args.bias
            else None
        )
        ...
        ratio = t_mojo / t_up
        print(
            f"{(batch, dim, seqlen, width)!s:>22} | {t_mojo:10.1f} | {t_up:14.1f} | {ratio:13.2f}x"
        )
