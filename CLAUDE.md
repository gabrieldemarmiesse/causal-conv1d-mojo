...
- `scripts/`: all benchmark drivers, perf gates, and dev tooling live
  ...
  - `assembly/nvidia/` (ours) and `reference_assembly/nvidia/`
    (upstream): committed PTX/SASS, regenerated by the master bench.
...
`scripts/_asm_tools.py`. The committed `scripts/assembly/nvidia/` (ours) and
`scripts/reference_assembly/nvidia/` (upstream) are regenerated by the master
bench; `scripts/reference_assembly/` only changes when the upstream wheel bumps
(`--refresh-reference`).
