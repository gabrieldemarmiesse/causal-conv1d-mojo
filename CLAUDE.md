...
- `scripts/`
  ...
  - `asm_tools.py`: PTX→SASS, ptxas `-v` spill canary, upstream-SASS
...
uv run --extra nvidia python benchmarks/bench.py fwd --shape 1,4096,2048,4 --impl all

# End-to-end wall-clock (torch.utils.benchmark, auto sync)
uv run --extra nvidia python benchmarks/bench.py update --shape 16,2048 --measure walltime
...
(e) dumps our PTX/SASS to `assembly/nvidia/`, (f) diffs it against
`reference_assembly/nvidia/` as an instruction-mix histogram, (g) runs
...
adds `-D DUMP_GPU_ASM=<dir>/<subpkg>__<mod>.ptx` to the `mojo build`, and
the stdlib's `compile_function` writes the PTX at runtime (the dump build
is its own cache entry, so the perf build is unaffected). SASS, the spill
report, and the histogram are all derived from that PTX by
`scripts/asm_tools.py`. The committed `assembly/nvidia/` (ours) and
`reference_assembly/nvidia/` (upstream) are regenerated by the master
bench; `reference_assembly/` only changes when the upstream wheel bumps
(`--refresh-reference`).
...
smsp__inst_executed_op_shared_ld.sum" \
    python benchmarks/bench.py fwd --shape 1,4096,2048,4 \
...
uv run --extra nvidia nsys profile --stats=true \
    -o /tmp/causal_conv1d \
    python benchmarks/bench.py fwd --shape 1,4096,2048,4 --impl mojo --measure raw
```
...
3. If all shapes regress → check the PTX for the relevant variant.
   ...
   hand: `CAUSAL_CONV1D_DUMP_ASM=$PWD/assembly/nvidia uv run --extra
   nvidia python benchmarks/bench.py <fn> --shape <S> --impl mojo
   --measure raw --iters 1 --warmup 0 --runs 1`, then
   `scripts/asm_tools.py sass|spill|histogram …`. No need to edit
