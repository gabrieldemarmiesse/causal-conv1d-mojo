...
The committed `scripts/assembly/nvidia/` (ours) and
`scripts/reference_assembly/nvidia/` (upstream `.sass` + `.ptx`) are
regenerated by the master bench; `scripts/reference_assembly/` only changes
when the upstream wheel bumps (`--refresh-reference`, which re-clones + recompiles).
