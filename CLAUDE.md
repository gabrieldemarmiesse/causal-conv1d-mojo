...
SASS, the spill report, and the histogram are all derived from that PTX by
`scripts/_asm_tools.py` using the triton wheel's portable `ptxas`/`nvdisasm`. The committed `scripts/assembly/nvidia/` (ours) and
